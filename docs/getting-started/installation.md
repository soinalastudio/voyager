...
```bash
php artisan vendor:publish --provider="SoinalaStudio\Voyager\VoyagerServiceProvider"
...
Now, open your User-Model \(usually `app/User.php`\) and make the class extend `\SoinalaStudio\Voyager\Models\User` instead of `Authenticatable`.

```php
<?php

class User extends \SoinalaStudio\Voyager\Models\User
{
    // ...
}
```
